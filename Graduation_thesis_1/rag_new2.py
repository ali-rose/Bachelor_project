...
DATA_PATH = '/root/autodl-tmp/glm4-9B/government_documents_falv_2025-01-28_21-12-13_adjust.xlsx'  # 数据集路径
EMBEDDING_PATH = '/root/autodl-tmp/glm4-9B/embedding_cashe/embeddings_law.npy'  # 嵌入向量缓存路径
METADATA_PATH = '/root/autodl-tmp/glm4-9B/embedding_cashe/metadata_law.pkl'  # 元数据缓存路径
INDEX_PATH = '/root/autodl-tmp/glm4-9B/embedding_cashe/policy_faiss_law.index'  # FAISS索引缓存路径
CATEGORY_EMBEDDING_PATH = '/root/autodl-tmp/glm4-9B/embedding_cashe/category_embeddings_law.npy'  # 类别嵌入向量缓存路径
TOP_K = 5  # 检索的文档数量
...
class ResponseEvaluator:
    ...
    def calculate_completeness_score(self, answer, context):
        try:
            rouge_scores = self.rouge.get_scores(answer, context)
            rouge_l = rouge_scores[0]["rouge-l"]["f"]
            return min(rouge_l * 10, 10)
        except:
            return 5.0
...
def generate_answer(question, index, metadata, category_embeddings, evaluator, top_k=TOP_K):
    try:
        ...
        if matched_categories:
            ...
            if len(filtered_metadata) < top_k:
        ...
        filtered_embeddings = np.array([
            get_embeddings_local(doc['正文'])
            for doc in filtered_metadata
        ]).astype('float32')

        
        print("filtered_embeddings------",filtered_embeddings)
        ...
        distances, indices = temp_index.search(question_vector, min(top_k, len(filtered_metadata)))
        retrieved_docs = [filtered_metadata[idx] for idx in indices[0]]
