...
plt.title('Classification of points with Noise')
plt.show()
...
Yt = Yt[Yt > 0]
m = len(Yt)
Ym_nb = np.zeros(m, dtype=int)
Ym_knn = np.zeros(m, dtype=int)
...
for i in range(1, 10):
    plt.scatter(Xt[Ym_nb == i, 0], Xt[Ym_nb == i, 1], c=colors[i-1], marker='+', facecolors='none', edgecolors=colors[i-1], label=f'Predicted Class {i} (NB)')
plt.xlabel('x axis')
plt.ylabel('y axis')
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
plt.title('Naive Bayes Classification with Noise')
plt.show()
...
plt.scatter(Xt[:, 0], Xt[:, 1], c='black', marker='+', label='Test Data', edgecolors='k')
for i in range(1, 10):
    plt.scatter(Xt[Ym_knn == i, 0], Xt[Ym_knn == i, 1], c=colors[i-1], marker='+', facecolors='none', edgecolors=colors[i-1], label=f'Predicted Class {i} (KNN)')
plt.xlabel('x axis')
plt.ylabel('y axis')
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
plt.title('K-Nearest Neighbors Classification with Noise')
plt.show()

# 计算错误率
error_rate_nb = np.mean(Ym_nb != Yt)
error_rate_knn = np.mean(Ym_knn != Yt)

print(f'朴素贝叶斯错误率: {error_rate_nb * 100:.2f}%')
print(f'K近邻错误率: {error_rate_knn * 100:.2f}%')
