...
# 准备数据
future_dates = pd.date_range(start='2015-08-26', periods=3)
...
# 准备数据
artist_forecast_2 = pd.DataFrame()
for artist in artist_daily_play_count['artist_id'].unique():
    artist_data_2 = artist_daily_play_count[artist_daily_play_count['artist_id'] == artist].set_index('record_date')
    artist_data_2 = artist_data_2.asfreq('D').fillna(0)  # 填补缺失值
    artist_data_2['play_count'] = artist_data_2['play_count_smooth']  # 使用平滑后的播放量数据

    # 训练ARIMA模型
    model = ARIMA(artist_data_2['play_count'], order=(5, 1, 0))  # ARIMA(p, d, q)
    model_fit = model.fit()

    # 预测未来3天
    forecast_2 = model_fit.forecast(steps=3)
    forecast_dates_2 = pd.date_range(start='2015-08-26', periods=3)
    forecast_df_2 = pd.DataFrame({'date': forecast_dates_2,
                                'predicted_play_count': forecast_2,
                                'artist_id': artist})

    artist_forecast_2 = pd.concat([artist_forecast_2, forecast_df_2], ignore_index=True)

# 重置索引
artist_forecast_2.reset_index(drop=True, inplace=True)

# 将预测播放数四舍五入到整数
artist_forecast_2['predicted_play_count'] = artist_forecast_2['predicted_play_count'].round().astype(int)
...
def evaluate_predictions(predictions, actuals):
    ...
    # 合并预测结果和实际数据
    merged = pd.merge(predictions, actuals, on=['artist_id', 'date'], suffixes=('_pred', '_actual'))

    # 计算每个艺人的 \(\sigma_{i,j}\)
    merged['normalized_error'] = ((merged['predicted_play_count'] - merged['play_count']) / merged['play_count']) ** 2
    artist_error = merged.groupby('artist_id')['normalized_error'].mean().apply(np.sqrt)

    # 计算每个艺人的权重 \(\phi_j\)
    artist_weight = actuals.groupby('artist_id')['play_count'].mean()

    # 计算参赛选手的预测分数 \(F_i\)
    artist_score = (1 - artist_error) * artist_weight
    total_score = artist_score.sum()

    return total_score

# 示例数据
predictions_xgboost = artist_forecast[['artist_id','date','predicted_play_count']].copy()
predictions_xgboost.columns=['artist_id','date','predicted_play_count']

predictions_arima =artist_forecast_2[['artist_id','date','predicted_play_count']].copy()
predictions_arima.columns=['artist_id','date','predicted_play_count']


actuals = artist_daily_play_count[['artist_id','record_date','play_count']].copy()
actuals.columns =['artist_id','date','play_count']

# 计算分数
score1 = evaluate_predictions(predictions_xgboost, actuals)
score2 = evaluate_predictions(predictions_arima, actuals)

print(f"XGboost的预测分数: {score1}")
print(f"Arima的预测分数: {score2}")
